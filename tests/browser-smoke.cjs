...
(async () => {
  ...
  try {
    ...
    assert.match(await page.locator('#slot-status').textContent(), /저장했어요/);
    assert.equal(await page.evaluate(() => JSON.parse(localStorage.getItem('headache-escape-slots-v1')).length), 5);
    ...
    console.log('PASS: Escape published subpath loads all modules and art, renders the 3D room, and saves/loads one of five local slots.');
